...
contract PeriodClaimVaultV2 is OwnableUpgradeable {
  ...
  event SetRewardPerPeriod(uint256 projectId, uint256 reward);
  ...
  function claim(uint256 _projectId) external returns (uint256) {
    ...
    uint256 _periodRewards = rewardPerPeriod[_projectId];
    require(_periodRewards > 0, "unset rewardPerPeriod");
  ...
  function setRewardPerPeriod(uint256 _projectId, uint256 _rewardPerPeriod) external onlyOwner {
    require(_rewardPerPeriod > 0, "invalid reward");
    require(projectRecipient[_projectId] != address(0), "invalid project");

    rewardPerPeriod[_projectId] = _rewardPerPeriod;
    emit SetRewardPerPeriod(_projectId, _rewardPerPeriod);
  }
